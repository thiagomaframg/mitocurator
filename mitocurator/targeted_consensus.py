...
def run_targeted_consensus(config, root: Path, refinement_dir: Path, reconstruction_pools_dir: Path, outdir: Path):
    ...
    max_depth_per_position = int(safe_get(config, ["targeted_consensus", "max_depth_per_position"], 1000))
    ...
    for r in df.itertuples():
        ...
        t0 = perf_counter()
        print(f"[targeted_consensus] target={tid} read_set={rs} step=prepare_reads")
        reads_available = 0
        reads_used = 0
        downsampled = "no"
        use_fq, use_fq1, use_fq2 = fq, fq1, fq2
        ds_dir = ensure_dir(tdir / "reads_downsampled")
        if max_reads is not None and max_reads > 0:
            if task_fmt == "paired_fastq" and fq1 != "." and fq2 != ".":
                r1 = _read_fastq(Path(fq1)); r2 = _read_fastq(Path(fq2))
                n = min(len(r1), len(r2)); reads_available = n
                if n > max_reads:
                    idx = sorted(rng.sample(range(n), max_reads))
                    r1s = [r1[i] for i in idx]; r2s = [r2[i] for i in idx]
                    use_fq1 = str(ds_dir / f"{tid}.{rs}.R1.downsampled.fastq.gz")
                    use_fq2 = str(ds_dir / f"{tid}.{rs}.R2.downsampled.fastq.gz")
                    _write_fastq(Path(use_fq1), r1s); _write_fastq(Path(use_fq2), r2s)
                    downsampled = "yes"; reads_used = len(r1s)
                else:
                    reads_used = n
            else:
                rr = _read_fastq(Path(fq)); reads_available = len(rr)
                if len(rr) > max_reads:
                    idx = sorted(rng.sample(range(len(rr)), max_reads))
                    rrs = [rr[i] for i in idx]
                    use_fq = str(ds_dir / f"{tid}.{rs}.downsampled.fastq.gz")
                    _write_fastq(Path(use_fq), rrs)
                    downsampled = "yes"; reads_used = len(rrs)
                else:
                    reads_used = len(rr)
        print(f"[targeted_consensus] target={tid} read_set={rs} original_reads={reads_available} reads_used_for_alignment={reads_used} max_reads={max_reads} downsampled={downsampled}")
        ...
        if not (reuse and out_bam.exists() and out_bai.exists() and cpath.exists()):
            ...
            print(f"[targeted_consensus] target={tid} read_set={rs} step=map")
            if task_fmt == "paired_fastq" and fq1 != "." and fq2 != ".":
                cmd = f"minimap2 -t {minimap2_threads} -ax {preset} {local_ref} {use_fq1} {use_fq2} | samtools sort -@ {samtools_threads} -o {out_bam}"
            else:
                cmd = f"minimap2 -t {minimap2_threads} -ax {preset} {local_ref} {use_fq} | samtools sort -@ {samtools_threads} -o {out_bam}"
            run_cmd(["bash", "-lc", cmd], check=False)
            print(f"[targeted_consensus] target={tid} read_set={rs} step=index")
        ...
        depths = []
        if aln is not None:
            print(f"[targeted_consensus] target={tid} read_set={rs} step=pileup")
            for p in range(len(local_seq)):
                counts = {"A": 0, "C": 0, "G": 0, "T": 0}
                depth = 0
                for col in aln.pileup(tid, p, p + 1, truncate=True, stepper="all", max_depth=max_depth_per_position):
        ...
        print(f"[targeted_consensus] target={tid} read_set={rs} step=orf")
        ncount = cseq.count("N")
