...
def _build_gene_problem_priority(root: Path) -> List[Dict[str, str]]:
    ...
    if p_refine.exists():
        ...
        if data:
            cols = list(data[0].keys())
            gene_col = _first_existing_column(cols, ["gene", "target", "gene_normalized", "target_gene"])
            problem_col = _first_existing_column(cols, ["problem", "issue_type", "issue", "category", "recommendation_type"])
            pri_col = _first_existing_column(cols, ["priority", "decision_hint", "status"])
            detail_col = _first_existing_column(cols, ["recommendation", "evidence_summary", "comment", "details"])
            for r in data:
    ...
    if p_targeted.exists():
        ...
        if data:
            ...
            status_col = _first_existing_column(cols, ["priority", "recommendation", "status", "consensus_status", "decision_hint"])
            for r in data:
    ...
    if p_cand.exists():
        ...
        if data:
            cols = list(data[0].keys())
            gene_col = _first_existing_column(cols, ["gene", "target_gene", "gene_normalized"])
            status_col = _first_existing_column(cols, ["priority", "recommendation", "candidate_region_status", "status", "decision_hint"])
            detail_col = _first_existing_column(cols, ["recommendation", "candidate_region_status", "comment", "summary"])
            for r in data:
