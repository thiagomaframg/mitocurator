...
def cmd_run(args):
    ...
    if fmt == "fasta":
        ...
        config["input"]["mitogenome"] = str(annotated_gb)
        print("[2/7] MitoFinder annotation: skipped (input already annotated GenBank)")
    ...
    try:
        ...
        print(f"[4/7] Rotation: {rotated_input}")
    except Exception as e:
        print(f"[4/7] Rotation skipped/failed: {e}")
    ...
    if read_support_enabled:
        ...
        print(f"[5/7] Read support: {rs_dir}")
    else:
        print("[5/7] Read support: disabled")
    ...
    if targeted_enabled:
        ...
        print(f"[6/7] Targeted extraction: {te_dir}")
    else:
        print("[6/7] Targeted extraction: disabled")

    qc_dir = ensure_dir(root / "07_gene_qc")
    diagnose(config, qc_dir)
    print(f"[7/7] Diagnosis: {qc_dir}")
