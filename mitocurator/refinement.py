...
def _pairwise_metrics(query_aa, ref_aa):
    ...
    if PairwiseAligner is not None:
        ...
        al = pairwise2.align.globalms(query_aa, ref_aa, 1, -1, -2, -0.5, one_alignment_only=True)
        if not al:
            return 0.0, 0.0, 0.0, 0.0
        q_aln, r_aln, score, _, _ = al[0]
    matches = sum(1 for a, b in zip(q_aln, r_aln) if a == b and a != "-" and b != "-")
    aligned = sum(1 for a, b in zip(q_aln, r_aln) if a != "-" and b != "-")
    pid = (100.0 * matches / aligned) if aligned else 0.0
    cov_q = (100.0 * sum(1 for a in q_aln if a != "-") / len(query_aa)) if query_aa else 0.0
    cov_r = (100.0 * sum(1 for b in r_aln if b != "-") / len(ref_aa)) if ref_aa else 0.0
...
def generate_reference_similarity_candidates(config, record, missing_candidates_tsv, out_tsv):
    ...
    for _, r in cand_df.iterrows():
        ...
        if not ref_path:
            ...
            elif pid >= 40 and covr >= 50:
                hint = "STRONG_REF_MATCH"
            elif pid >= min_id:
                hint = "PARTIAL_REF_MATCH"
            row.update({"reference_gene": gene, "reference_aa_length": len(ref_aa), "length_ratio": ratio, "percent_identity": pid, "aligned_coverage_candidate": covq, "aligned_coverage_reference": covr, "alignment_score": score, "decision_hint": hint, "comment": "protein alignment to reference CDS"})
        rows.append(row)
    pd.DataFrame(rows, columns=cols).to_csv(out_tsv, sep="	", index=False)
